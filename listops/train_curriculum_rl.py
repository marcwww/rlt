...
from listops import model, model_rl
import tqdm
...
def run_iter(model, criterion, optimizer,
             params, batch, is_training,
             train_parser=False):
    model.train(is_training)
    expr, length = batch.expr
    val = batch.val
    clf_logits, log_prob_sum = model(inp=expr, length=length)
    # print(length)
    pred = clf_logits.max(dim=1)[1]
    acc = torch.eq(val, pred).float().mean()
    score = criterion(input=clf_logits, target=val)

    if train_parser:
        reward = torch.eq(val, pred).float()
        reward_normalized = (reward - torch.mean(reward)).div(torch.std(reward))
        loss = -(reward * log_prob_sum).mean()
        # score.detach_()
        # score_mean = torch.mean(score)
        # score_std = torch.std(score)
        # score_normalized = (score - score_mean).div(score_std)
        # loss = torch.mean(score_normalized * log_prob_sum) # no negative operator, for that is included in the score
    else:
        score_mean = torch.mean(score)
        loss = score_mean
...
def train(args):
    ...
    model = model_rl.ListopsModel(nclasses=len(VAL.vocab),
    ...
    for srange, train_iter in train_iters.items():
        ...
        for epoch in range(args.nepoches):
            ...
            for i, batch in enumerate(train_iter_tqdm):
                ...
                if train_parser:
                    train_losses_parser.append(loss.item())
                else:
                    train_losses_comp.append(loss.item())

                if (i + 1) % valid_every == 0:
                    print_str = f'Split range {srange} ' \
                                f'Epoch {epoch} ' \
                                f'Train loss(comp) {np.mean(train_losses_comp):.4f} ' \
                                f'Train loss(parser) {np.mean(train_losses_parser):.4f} ' \
                                f'Train acc {np.mean(train_accs):.4f} '
                    train_iter_tqdm.write(print_str)
